...
const PSO_PANEL_CONFIG = {
  ...
  description: 'Particles search for the global minimum of a fitness function using velocity updates inspired by social behavior.',
  sliders: [
    { section: 'Fitness Function', key: 'functionIndex', label: 'Function (0-3)', min: 0, max: 3, step: 1, decimals: 0 },
    { section: 'Velocity Weights', key: 'inertiaWeight', label: 'Inertia (w)', min: 0.1, max: 1.5, step: 0.05, decimals: 2 },
    ...
    { section: 'Swarm', key: 'particleCount', label: 'Particles', min: 5, max: 100, step: 1, decimals: 0 },
...
function buildPsoPanel(
  ...
  fnLabel.style.cssText = 'font-size:11px;color:#5588ff;margin-top:-4px;margin-bottom:8px;padding-left:2px;';
...
function createPsoSimulation(
  ...
  const { pauseBtn, saveBtn, getParams } = buildPsoPanel(panel, { ...DEFAULT_PARAMS }, {
    onParamsChange: (newParams: PsoParams) => { state = updateParams(state, newParams); },
    onPause: () => {
      loop.toggle();
      pauseBtn.textContent = loop.isRunning() ? 'Pause' : 'Resume';
    },
    onReset: () => { state = resetParticles(state); },
    onFunctionChange: (index: number) => { state = setFunction(state, index); },
    onSave: () => { handlePsoSave(getParams, saveBtn); },
  });
...
register({
  id: 'pso',
  title: 'Particle Swarm Optimization',
  description: 'A swarm of particles searches for the global minimum of a fitness landscape. Each particle tracks its personal best and the swarm\'s global best, converging via velocity updates. Kennedy & Eberhart, 1995.',
