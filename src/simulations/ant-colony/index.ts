...
function buildAcoPanel(
	...
	return buildPanel(panel, ACO_PANEL_CONFIG, params as unknown as Record<string, number>, {
 ...
 */
function createAcoSimulation(canvas: HTMLCanvasElement, panel: HTMLElement): SimulationInstance {
	...
	const { pauseBtn, saveBtn, getParams } = buildAcoPanel(panel, DEFAULT_PARAMS, {
		onParamsChange: (newParams: AcoParams) => {
		...
		onReset: () => {
			state = resetAnts(state);
		},
		onSave: () => {
			handleAcoSave(getParams, saveBtn);
		},
	});
...
register({
	...
	title: 'Ant Colony Optimization',
	description:
		'Ants find shortest paths between nest and food source using pheromone trails. Inspired by the foraging behavior of real ants -- no central planner, just local chemical signals.',
