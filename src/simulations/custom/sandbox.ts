...
export class SimulationSandbox {
  ...
  private tickFn: ((state: unknown, dt: number, ctx: Record<string, unknown>) => unknown) | null = null;
  ...
  compile(code: string): void {
    ...
    try {
      ...
      if (!result.init) { this.error = 'Missing init() function'; return; }
      if (!result.tick) { this.error = 'Missing tick() function'; return; }
  ...
  tick(dt: number, ctx: CanvasRenderingContext2D): void {
    ...
    try {
      ...
      this.lastCommands = commands.length > MAX_COMMANDS
